...
The managed identity has one job: read the DB password from Key Vault.
**No database role bootstrap step is needed** — the app logs in with the PG
admin credentials the server already knows about.
...
az containerapp create \
  ...
  --secrets \
    "db-password=keyvaultref:${KV_URI}secrets/db-admin-password,identityref:${MI_ID}" \
    "ghcr-pat=$GHCR_PAT" \
  --registry-server ghcr.io \
  --registry-username "$GHCR_OWNER" \
  --registry-password-secret-name ghcr-pat \
...
— no client secret stored in the repo. The Docker image is pushed to GHCR
using the built-in `GITHUB_TOKEN` (no PAT required for the push step).
