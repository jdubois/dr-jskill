...
8. Follow daily Git best practices for small branches, reviewed diffs, safe commits, pull requests, and worktrees - see [Git Best Practices](references/GIT.md)
   - For parallel Git worktrees, open each one in its own `.devcontainer/`. Dev Containers keep Spring Boot, Vite, and PostgreSQL on their standard internal ports without Git hooks or per-worktree `.env` rewriting.
